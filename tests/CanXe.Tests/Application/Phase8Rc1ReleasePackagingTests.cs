...
public sealed class Phase8Rc1ReleasePackagingTests
{
    ...
    public void Desktop_Version_Is_1_0_2()
    {
        ...
        Assert.Contains("<Version>1.0.4</Version>", text, StringComparison.Ordinal);
        Assert.Contains("<InformationalVersion>1.0.4</InformationalVersion>", text, StringComparison.Ordinal);
    ...
    public void InnoSetup_Script_PreservesUserDataAndTargetsPublish()
    {
        ...
        Assert.Contains("#define MyAppVersion \"1.0.4\"", text, StringComparison.Ordinal);
        Assert.Contains("CanXeTienTrong-Setup-1.0.4", text, StringComparison.Ordinal);
    ...
    {
        var notes = ResolveFile(Path.Combine("releases", "ReleaseNotes-1.0.4.txt"));
        var text = File.ReadAllText(notes);
        Assert.Contains("Cân Xe Tiến Trọng 1.0.4", text, StringComparison.Ordinal);
